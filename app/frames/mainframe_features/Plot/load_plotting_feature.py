...
def load_plotting_features(self, parent):
    ...
    self.previewButtonPLT.setGeometry(QtCore.QRect(5, 20, 88, 34))
    self.previewButtonPLT.setText("Preview")
    self.previewButtonPLT.clicked.connect(lambda: show_dataframe_sample(self))

    self.plottingOptionLabel = QtWidgets.QLabel(self.PlottingGB)
    self.plottingOptionLabel.setGeometry(QtCore.QRect(5, 60, 150, 30))
    self.plottingOptionLabel.setText("Plot type:")

    self.plottingOptions = QtWidgets.QComboBox(self.PlottingGB)
    self.plottingOptions.setGeometry(QtCore.QRect(5, 90, 150, 30))
